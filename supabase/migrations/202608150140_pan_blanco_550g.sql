-- CIBUSPAN ONE
-- Formula inicial idempotente de PAN BLANCO 550G y su micro.
...
begin
  ...
  values (
    'PAN-BLANCO-550',
    'PAN BLANCO 550G',
    'Pan blanco de 550 g. Una parada produce 100 panes.',
    true
  )
  on conflict (codigo) do update
  ...
  values (
    'MIC-PAN-BLANCO-550',
    'MICRO PAN BLANCO 550G',
    'Micro de azucar, sal, ECOFRESH, MOHOSORBIC y propionato.',
    true
  )
  on conflict (codigo) do update
  set nombre = excluded.nombre,
  ...
  if v_version_id is null then
    ...
    values (
      ...
      1,
      15.384615,
      '100 panes por parada. Peso calculado del micro: 4.370 kg. La funda 00030 se comparte con la referencia Blanco 600 g.'
    )
  ...
  if v_componentes = 0 then
    ...
    if v_faltantes is not null then
      raise exception 'Faltan materias primas para PAN BLANCO 550G: %', v_faltantes;
    end if;
    ...
    select
      ...
      receta.porcentaje,
      receta.destino,
      case when receta.destino = 'MICRO' then v_micro_id else null end,
      receta.es_base,
      receta.costea,
      receta.orden,
    ...
    from (
      values
        ...
        ('00015',  10.000000::numeric, 'DIRECTO', false, true,  2, 'MANTECA'),
        ('00017',   0.600000::numeric, 'DIRECTO', false, true,  3, 'VINAGRE'),
        ('00016',   0.750000::numeric, 'DIRECTO', false, true,  4, 'LEVADURA'),
        ('00013',   8.000000::numeric, 'MICRO',   false, true,  5, 'AZUCAR'),
        ('00014',   2.200000::numeric, 'MICRO',   false, true,  6, 'SAL'),
        ('00020',   1.500000::numeric, 'MICRO',   false, true,  7, 'ECOFRESH'),
        ('00124',   0.400000::numeric, 'MICRO',   false, true,  8, 'MOHOSORBIC'),
        ('00021',   0.200000::numeric, 'MICRO',   false, true,  9, 'PROPIONATO'),
        ('AGUA',    59.400000::numeric, 'DIRECTO', false, false, 10, 'AGUA SIN COSTO')
    ) as receta(
      codigo,
      porcentaje,
      destino,
    ...
    join lateral (
      ...
      order by
        case
          when upper(coalesce(candidata.codigo_contable, '')) = receta.codigo then 1
          else 2
        end,
        candidata.creado_en
      limit 1
  ...
  where trim(producto.codigo) = '7868304276292'
  limit 1;

  if v_producto_id is null then
    raise exception 'No se encontro el SKU PAN BLANCO 550G (7868304276292).';
  end if;
  ...
  into v_funda_id
  from public.materias_primas materia
  where upper(coalesce(materia.codigo_contable, '')) = '00030'
     or upper(coalesce(materia.codigo, '')) = '00030'
  order by
    case
      when upper(coalesce(materia.codigo_contable, '')) = '00030' then 1
      else 2
    end,
    materia.creado_en
  limit 1;
  ...
  if v_funda_id is null then
    raise exception 'No se encontro la funda compartida BLANCO 550/600G (00030).';
  end if;
  ...
  values (
    ...
    1,
    true,
    'Una funda 00030 por cada Pan Blanco 550 g.'
  )
  on conflict (producto_id, materia_prima_id) do update
  set cantidad_por_unidad = excluded.cantidad_por_unidad,
      activo = true,
