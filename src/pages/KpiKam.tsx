  ...
  return (
    <section className="kam-page">
      ...
      <header className="kam-head">
        ...
        <button type="button" onClick={() => setActualizacion((valor) => valor + 1)} disabled={cargando}>
        ...
        <>
          ...
          <section className="kam-cards">
            {resumen.detalles.map((detalle) => {
              ...
              return (
                <article key={detalle.codigo} className={`kam-card ${detalle.estado.toLowerCase()}`}>
                  <header><span>{detalle.nombre}</span>{detalle.alertas > 0 && <b>{detalle.alertas} alerta{detalle.alertas === 1 ? "" : "s"}</b>}</header>
                  <strong>{detalle.estado === "NO_APLICA" ? "N/A" : porcentaje(detalle.valor)}</strong>
                  <div><span>{detalle.nota == null ? "Sin nota" : `${detalle.nota.toFixed(0)}/100`}</span><b>{detalle.puntos == null ? "—" : `${detalle.puntos.toFixed(1)} pts`}</b></div>
                  <small className={mejora == null ? "neutral" : mejora ? "positivo" : "negativo"}>
          ...
          <section className="kam-tabla-panel">
            ...
            <div className="kam-tabla-wrap">
              <table>
                <thead><tr><th>Cliente</th><th>Puntaje</th><th>Ventas</th><th>Margen</th><th>Devoluciones</th><th>Fugas</th><th>Cobertura</th><th>Compromisos</th></tr></thead>
                <tbody>
