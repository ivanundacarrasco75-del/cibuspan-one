...
export async function obtenerCatalogoProductosDevolucionDb() {
  ...
  if (error) {
    throw new Error(
      `No se pudo cargar el catÃ¡logo de SKU: ${error.message}`,
    )
  }
...
export async function registrarDevolucionDb(
  ...
  if (!datos.fechaDevolucion) {
    throw new Error(
      "La fecha de devoluciÃ³n es obligatoria.",
    )
  }
...
export async function obtenerDevolucionesDb(
  ...
  while (true) {
    ...
    const bloquesIds: string[][] = []

    for (let indice = 0; indice < ids.length; indice += 100) {
      bloquesIds.push(ids.slice(indice, indice + 100))
    }

    const resultadosDetalles = await Promise.all(
      bloquesIds.map((bloqueIds) =>
        supabase
    ...
    for (const { data: detallesData, error: detallesError } of resultadosDetalles) {
      if (detallesError) {
        throw new Error(
