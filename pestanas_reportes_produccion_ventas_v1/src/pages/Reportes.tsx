...
export default function Reportes({ cambiarPantalla }: ReportesProps) {
    ...
    () => {
      ...
      pedidosCliente.forEach(({ pedido, detalles }) => {
        ...
        detalles.forEach((detalle) => {
          ...
          punto.ventasNetas += unidades * precio
          punto.contribucion +=
            unidades * (precio - costoUnitario)
        })
      })
      ...
      ({ devolucion, detalle }) => {
        ...
        if (existente) {
          ...
          resumen.set(id, {
            ...
            despachos: 0,
            unidades: 0,
            devueltas: Number(detalle.unidades ?? 0),
          })
        }
      },
    )
    ...
    <div className="c1-reportes" style={pagina}>
      ...
      <header className="reports-header">
        <div>
          ...
          <h1>Reportes</h1>
          <p>
            Pedidos, despachos, producción y devoluciones
            analizados por semanas de lunes a domingo.
          </p>
        </div>
      </header>

      <nav className="reports-main-tabs" aria-label="Tipos de reportes">
        <button type="button" className="active">
          Producción
        </button>
        <button
          type="button"
          onClick={() => cambiarPantalla("Ventas por cliente y SKU")}
        >
          Ventas
        </button>
      </nav>
      ...
      <section className="reports-kpis" style={tarjetas}>
        ...
        <Tarjeta
          ...
          detalle={`${unidadesProducidas} unidades · ${paradasProducidas} paradas`}
        />
        <Tarjeta
          titulo="Fill Rate"
          valor={`${fillRate}%`}
              ...
              <article className="wide">
                <div className="reports-chart-heading">
                  <div>
                    ...
                    <p>
                      Precio vigente − devoluciones − costo de
                      materia prima
                    </p>
                  </div>
                </div>
      ...
      <section
        ...
        style={panel}
      >
        <h2>Despachos y devoluciones por cliente</h2>
      ...
      <svg viewBox={`0 0 ${ancho} ${alto}`} role="img" aria-label="Evolución semanal de despachos y devoluciones">
        ...
        {datos.map((punto, indice) => (
          <g key={punto.semanaInicio}>
            ...
            <text x={x(indice)} y={alto - 18} textAnchor="middle" className="commercial-axis-label">S{numeroSemanaIso(punto.semanaInicio)}</text>
          </g>
        ))}
      </svg>
        ...
        {datos.map((punto, indice) => {
          ...
          return (
            <g key={punto.semanaInicio}>
              ...
              <text x={centroX} y={alto - 19} textAnchor="middle" className="commercial-axis-label">S{numeroSemanaIso(punto.semanaInicio)}</text>
            </g>
          )
        })}
      </svg>
      ...
      >
        ...
        {datos.map((punto, indice) => (
          <text
            ...
            fill="#8e7c75"
          >
            S{numeroSemanaIso(punto.semanaInicio)}
          </text>
        ))}
